...
/** Letters stored in `cc1-ms-solutions` `moves` arrays. `W` = idle monster tick. */
export type SolutionMoveLetter = "U" | "D" | "L" | "R" | "W";

/** Decoded autoplay / simulation step. */
...
export function isWaitAction(action: SolutionAction): action is "wait" {
  return action === "wait";
}
