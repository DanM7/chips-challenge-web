...
function run(s: string) {
  ...
  for (const c of s) {
    ...
    console.log(c, [t.gx, t.gy], "tools", t.playerState.tools, "keys", t.playerState.keys, "bomb", cellTile(t.level,"upper",24,12), t.playerDied ? t.deathMessage : "");
    if (t.playerDied) break;
  }
}

run("UUURRRRRRDRDLLLL");
