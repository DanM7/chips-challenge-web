/**
 * chips5 → red@30,1 (27) → open exit red → finish chips → brown+exit ≥89 → pad to 89
 */
...
const steps: { label: string; done: (x: Runner) => boolean }[] = [
  { label: "chips5", done: (x) => x.playerState.chipsRemainingOnMap <= 5 && x.gx === 18 && x.gy === 6 },
  {
    label: "red301",
    done: (x) => x.playerState.keys.includes("key_red") && x.gx === 30 && x.gy === 1,
  },
  ...
  // Prefer block on brown for MS-valid exit
  {
    label: "brown",
...
for (const s of steps) {
  ...
  // chips5: allow any chips5 if exact pos fails
  let done = s.done;
  if (s.label === "chips5") {
    done = (x) => x.playerState.chipsRemainingOnMap <= 5;
  }
  if (s.label === "red301") {
    done = (x) => x.playerState.keys.includes("key_red");
  }
  let seg = bfsPath(r, done, s.label);
...
if (r.completed && finalRem > BOLD) {
  ...
  while (finalRem > BOLD && finalTicks < targetLow + 4 && guard++ < 150) {
    const i = Math.max(0, finalLetters.length - 35);
