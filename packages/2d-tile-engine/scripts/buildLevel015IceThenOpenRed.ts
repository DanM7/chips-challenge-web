/**
 * After ice: grab red, open exit door 16,15, then chips→brown→exit.
 */
...
function mazeKeySimple(r: Runner): string {
  let blk = "";
  for (const x of [15, 16, 17, 18, 19, 20])
    for (const y of [9, 10, 11, 12, 13, 14, 15])
      if (getCompositeTile(r.level, x, y) === "block_movable") blk += `${x},${y};`;
  return `${r.gx},${r.gy}|${r.playerState.chipsRemainingOnMap}|${r.playerState.keys.join("+")}|${r.playerState.tools.join("+")}|${blk}|${cellTile(r.level, "upper", 16, 15)}|${cellTile(r.level, "upper", 16, 11)}|${isTrapOpen(r.buttonPressCtx, 16, 16) ? 1 : 0}`;
}
...
function bfsPath(
  ...
  const seen = new Set([mazeKeySimple(start)]);
  let qi = 0;
...
// Cut first ice-complete: 4 boots, blue key, chips 6, bombs clear
{
  ...
  if (cut < 0) throw new Error("ice cut missing");
  var letters = all.letters.slice(0, cut);
}
let r = applyLetters(letters);
save(letters, "ice", r);

const steps: { label: string; done: (x: Runner) => boolean; nodes?: number }[] = [
  {
    label: "red",
    done: (x) => x.playerState.keys.includes("key_red") && x.playerState.keys.includes("key_blue"),
  },
  {
    label: "open_red_door",
...
for (const s of steps) {
  ...
  let seg = bfsPath(r, s.done, s.label, s.nodes ?? 300000);
