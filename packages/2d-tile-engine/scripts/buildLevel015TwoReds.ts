/**
 * Two red keys before south; blue; chips0; brown; exit 89.
 */
...
function mazeKey(r: Runner): string {
  ...
  return [
    ...
    cellTile(r.level, "upper", 18, 15)[0],
...
console.log("Searching 2 reds");
{
  const seg = bfs(
    r,
    80,
    600_000,
    (x) => redCount(x) >= 2 && x.playerState.tools.includes("ice_skates") && x.playerState.chipsRemainingOnMap <= 4,
    "2red",
  );
  if (!seg) {
    console.error("FAIL 2red");
    process.exit(1);
  }
...
console.log("Searching chips3 keep 2red");
{
  const seg = bfs(
    r,
    80,
...
console.log("Searching chips2+blue keep ≥1 red");
{
  const seg = bfs(
    r,
    150,
...
console.log("Searching chips0 keep blue+red");
{
  const seg = bfs(
    r,
    100,
...
console.log("Searching brown");
{
  const seg = bfs(
    ...
    100,
    1_000_000,
    (x) => getCompositeTile(x.level, 16, 9) === "block_movable" && redCount(x) >= 1,
    "brown",
  );
  if (!seg) {
    ...
    `${JSON.stringify(
      {
        ...
        boldRouteHint:
          "SW/NW boots; fire/ice; 2 reds; blue; last chips; brown; exit; 89",
        moveVerified: true,
