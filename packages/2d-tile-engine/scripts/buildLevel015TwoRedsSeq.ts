/**
 * Sequenced: chips4 → red(30,1) → red(18,4) → chips3 → c2+blue keep1red → chips0 → brown → exit
 */
...
const goals: { label: string; depth: number; nodes: number; done: (x: Runner) => boolean }[] = [
  {
    ...
    depth: 50,
    nodes: 300_000,
    done: (x) => redCount(x) >= 1 && x.gx === 30 && x.gy === 1,
  },
  {
    label: "red2",
    depth: 80,
  ...
  },
  {
    ...
    depth: 80,
    nodes: 600_000,
    done: (x) => x.playerState.chipsRemainingOnMap <= 3 && redCount(x) >= 2,
  },
  {
    label: "c2b",
    depth: 160,
  ...
  },
  {
    ...
    nodes: 1_000_000,
      ...
      x.playerState.chipsRemainingOnMap === 0 && blueCount(x) >= 1 && redCount(x) >= 1,
  },
  {
    label: "brown",
    depth: 100,
    nodes: 1_000_000,
    done: (x) => getCompositeTile(x.level, 16, 9) === "block_movable" && redCount(x) >= 1,
  },
  {
    label: "exit89",
    depth: 60,
