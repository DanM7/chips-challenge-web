...
for (const s of ["DRRULLLDLULL", "DRRULLLDLUL", "DRRULLLLDLULL", "DRRU LLL DL U LL".replace(/ /g, "")]) {
...
console.log("\ntrace:");
{
  ...
  for (const c of "DRRULLLDLULL") {
    stepMsCc1Simulation(
      t,
      (c === "U" ? "up" : c === "D" ? "down" : c === "L" ? "left" : "right") as Direction,
    );
    console.log(c, [t.gx, t.gy], "b="+blockPos(t), "bomb="+cellTile(t.level,"upper",24,14), "keys="+t.playerState.keys.join(","), t.playerDied?t.deathMessage:"");
    if (t.playerDied) break;
  }
}

const keySeq = "DRRULLLDLULL"; // adjust after trace
const afterKey = applyLetters([...letters, ...keySeq]);
if (
  afterKey.playerDied ||
  cellTile(afterKey.level, "upper", 24, 14) === "bomb" ||
  !afterKey.playerState.keys.includes("key_blue")
) {
  console.error("key seq failed, abort");
  process.exit(1);
}
letters.push(...keySeq.split(""));
let r = afterKey;
console.log("after ice key", {
  pos: [r.gx, r.gy],
