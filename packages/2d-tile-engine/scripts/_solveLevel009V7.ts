...
function stateKey(r: MsCc1SimulationRunner, monsters: boolean): string {
  ...
  if (monsters) {
    for (const m of r.monsters) parts.push(`m${m.kind[0]}${m.x},${m.y}`);
  }
  return parts.join("|");
}
...
function bfs(
  ...
  while (q.length && nodes < maxNodes) {
    ...
    nodes++;
    if (done(f.r)) {
      console.log("bfs", nodes, toLetters(f.seq).join(""), `->${f.r.gx},${f.r.gy}`);
      return f.seq;
    }
...
function go(seq: Act[], label: string): void {
  ...
  console.log(
    label,
    toLetters(seq).join(""),
    `pos=${runner.gx},${runner.gy} chips=${runner.playerState.chipsRemainingOnMap} keys=${runner.playerState.keys.join("+") || "-"}`,
...
// Green twice via U D (force boost style)
{
  const toGreen = bfs(
    runner,
    40,
    200_000,
    (r) => getCompositeTile(r.level, r.gx, r.gy) === "button_green",
  );
  if (!toGreen) throw new Error("green1");
  go(toGreen, "green1");
  go(expand("UD"), "green2"); // second press
}

// Blue key with waits for fireballs
{
  const seq = bfs(
    runner,
    60,
    800_000,
    (r) => r.playerState.keys.some((k) => k.includes("blue")),
    { wait: true, monsters: true },
  );
  if (!seq) throw new Error("blue");
  go(seq, "blue");
}

// Key chain: yellow, green, red keys via doors + buttons
{
  const seq = bfs(
    runner,
    100,
  ...
  if (!seq) throw new Error("chain-red");
  go(seq, "chain-red");
}

// Through red door at 24,15 and ice slide to bugs (with wait 1/2)
{
  const seq = bfs(
    runner,
    80,
  ...
  if (!seq) throw new Error("to-bugs");
  go(seq, "to-bugs");
}
    ...
    `${JSON.stringify(
      {
        ...
        boldRouteHint:
          "Yellow → force E → block bridges (dry dirt) → red ice → green×2 → key chain → bugs/sokoban → socket → block on brown → exit",
        moveVerified: rem === 306,
        meetsBoldBudget: rem >= 306,
        moveSource: `StrategyWiki + segmented BFS; rem ${rem} (bold 306)`,
