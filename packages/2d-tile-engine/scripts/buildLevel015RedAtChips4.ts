/**
 * Cut at chips4, grab red nearby, continue chips3→blue→chips0→brown→exit.
 */
...
function cutAt(
  pred: (r: Runner, i: number) => boolean,
): string[] {
  ...
  for (let i = 0; i < all.letters.length; i++) {
    ...
    if (pred(rs, i)) return all.letters.slice(0, i + 1);
  }
...
console.log("Searching red near chips4");
{
  const seg = bfs(
    r,
    40,
    ...
    (x) =>
      ...
      x.playerState.chipsRemainingOnMap <= 4 &&
...
for (const [label, target, depth] of [
  ["chips3", 3, 80],
  ["chips2", 2, 120],
] as const) {
  console.log("Searching", label);
  const needBlue = target <= 2;
  const seg = bfs(
    r,
    depth,
    1_200_000,
    (x) =>
      x.playerState.chipsRemainingOnMap <= target &&
      x.playerState.keys.includes("key_red") &&
      (!needBlue || x.playerState.keys.includes("key_blue")) &&
      x.playerState.tools.includes("ice_skates"),
    label,
  );
  if (!seg) {
...
// Ensure blue if chips2 didn't require it somehow
    ...
    `${JSON.stringify(
      {
        ...
        boldRouteHint:
          "SW/NW boots; fire/ice; red@chips4; blue; last chips; brown; exit; 89",
        moveVerified: true,
