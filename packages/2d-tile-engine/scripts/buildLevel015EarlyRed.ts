/**
 * chips3 → quick red → chips2+blue → chips0 keep keys → brown → exit 89
 */
...
) as { letters: string[] };
{
  ...
  for (let i = 0; i < all.letters.length; i++) {
    ...
    if (
      cut < 0 &&
      rs.playerState.chipsRemainingOnMap === 3 &&
...
console.log("Searching early red");
{
  const seg = bfs(
    r,
    60,
...
console.log("Searching chips2_blue keep red");
{
  const seg = bfs(
    r,
    150,
...
console.log("Searching chips0 keep keys");
{
  const seg = bfs(
    r,
    100,
    ...
    `${JSON.stringify(
      {
        ...
        boldRouteHint:
          "SW/NW boots; fire/ice; early red; blue; last chips; brown; exit; 89",
        moveVerified: true,
