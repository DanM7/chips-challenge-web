...
function roomKey(r: MsCc1SimulationRunner): string {
  ...
  return `${r.gx},${r.gy}|${b.join(";")}|c${r.playerState.chipsRemainingOnMap}|k${r.playerState.keys.join(",")}`;
}

function dump(r: MsCc1SimulationRunner, label: string): void {
  console.log(
    label,
    `pos=${r.gx},${r.gy} water=${waterN(r.level)} chips=${r.playerState.chipsRemainingOnMap} keys=${r.playerState.keys.join("+") || "-"} died=${r.playerDied}`,
    r.deathMessage ?? "",
  );
  for (let y = 19; y <= 28; y++) {
    let row = `${y}: `;
...
function bfs(
  ...
  const q: { seq: Act[]; r: MsCc1SimulationRunner }[] = [
  ...
  while (q.length && nodes < maxNodes) {
    const f = q.shift()!;
    nodes++;
    if (done(f.runner ?? f.r)) {
      const runner = f.runner ?? f.r;
      console.log("bfs", nodes, toLetters(f.seq).join(""));
      return f.seq;
    }
    const runner = (f as { r: MsCc1SimulationRunner }).r;
    if (runner.playerDied || f.seq.length >= maxDepth) continue;
    const acts: Act[] = allowWait ? [...dirs, "wait"] : [...dirs];
    for (const a of acts) {
      const next = cloneMsCc1SimulationRunner(runner);
...
// Fix typing - use consistent r field
function bfs2(
  start: MsCc1SimulationRunner,
  maxDepth: number,
  maxNodes: number,
...
// Manual side-push attempts for remaining waters
const manuals = [
  "4DL5ULUR", // plan: south of block, push up, side push into water
  "4DL4ULUR",
  "4DL6ULUR",
  "4DL5U L U R",
  "4DL5ULUUR",
...
for (const m of manuals) {
  const t = cloneMsCc1SimulationRunner(runner);
  applyActs(t, expand(m));
  console.log(
    "MAN",
    m,
  ...
  if (!t.playerDied && waterN(t.level) < 3) {
    dump(t, "GOOD " + m);
  }
}

// If manual works, use best; else BFS preferring not ending on water column mid-push
{
  let seq = bfs2(runner, 60, 3_000_000, (r) => waterN(r.level) < 3);
  if (!seq) throw new Error("b2");
  // Prefer non-drowning: already filtered playerDied
  go(seq, "b2");
}

{
  const seq = bfs2(runner, 60, 3_000_000, (r) => waterN(r.level) < 2);
  if (!seq) throw new Error("b3");
  go(seq, "b3");
}

{
  const seq = bfs2(runner, 60, 3_000_000, (r) => waterN(r.level) < 1);
  if (!seq) throw new Error("b4");
  go(seq, "b4");
}

{
  const seq = bfs2(runner, 30, 200_000, (r) =>
...
console.log("FULL ROUTE TO RED", toLetters(route).join(""));
console.log("letters", toLetters(route).length);

// Continue ice + rest with broader key including monsters for safety later
...

{
  ...
  if (!trial.playerDied) go(ice, "ice");
    ...
    const seq = bfsFull(runner, 80, 1_000_000, (r) => r.gx >= 19 && r.gy <= 22);
    ...
    JSON.stringify(
      {
        ...
        boldRouteHint:
          "Yellow key → lock → hold E → blocks to water → red ice → green×2 → key chain → bugs/blocks/bombs → socket → block on brown → exit",
        ...
        boldGapNote: rem === 306 ? undefined : `rem ${rem} vs bold 306`,
      },
      null,
      2,
    ) + "\n",
