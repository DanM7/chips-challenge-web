...
function sk(r: MsCc1SimulationRunner, monsters: boolean): string {
  const p: string[] = [`${r.gx},${r.gy}`, `c${r.playerState.chipsRemainingOnMap}`];
  for (let y = 0; y < 32; y++) {
    for (let x = 0; x < 32; x++) {
      const t = getCompositeTile(r.level, x, y);
      if (t === "block_movable") p.push(`b${x},${y}`);
      if (t === "bomb") p.push(`o${x},${y}`);
      if (t === "chip") p.push(`c${x},${y}`);
      if (isDirtCell(r.level, x, y)) p.push(`d${x},${y}`);
    }
  }
  if (monsters) for (const m of r.monsters) p.push(`m${m.kind[0]}${m.x},${m.y}`);
  return p.join("|");
}
...
function bfs(
  ...
  const seen = new Set([sk(start, monsters)]);
  let nodes = 0;
  while (q.length && nodes < maxNodes) {
    ...
    for (const a of [...dirs, "wait"] as Act[]) {
      ...
      const k = sk(n, monsters);
      if (seen.has(k)) continue;
      seen.add(k);
      q.push({ seq: [...f.seq, a], r: n });
    }
  }
  console.error("fail", nodes);
  return null;
}

function bombs(level: LevelData): number {
  let n = 0;
  for (let y = 0; y < 32; y++) for (let x = 0; x < 32; x++) if (getCompositeTile(level, x, y) === "bomb") n++;
  return n;
}
...
function go(seq: Act[], label: string): void {
  ...
  console.log(
    label,
    toLetters(seq).join(""),
    `pos=${runner.gx},${runner.gy} chips=${runner.playerState.chipsRemainingOnMap} bombs=${bombs(runner.level)}`,
...
go(expand("DDLDDLDLLLUUURRUUUUUULLLLULLDDDDDDDDDDDDDDDDDDRRUU"), "left-chip");

// Reduce chips / bombs until done
while (runner.playerState.chipsRemainingOnMap > 0 || !runner.completed) {
  if (runner.completed) break;
  const beforeC = runner.playerState.chipsRemainingOnMap;
  const beforeB = bombs(runner.level);

  // Prefer chip pickup
  let seq = bfs(runner, 100, 1_200_000, (r) => r.playerState.chipsRemainingOnMap < beforeC);
  if (seq) {
    go(seq, `chip-${beforeC}`);
    continue;
  }
  // Else blow a bomb
  if (beforeB > 0) {
    seq = bfs(runner, 80, 1_000_000, (r) => bombs(r.level) < beforeB);
    if (seq) {
      go(seq, `bomb-${beforeB}`);
      continue;
    }
  }
  // Else try exit if chips done
  if (beforeC === 0) {
    seq = bfs(runner, 80, 800_000, (r) => r.completed, true);
    if (seq) {
      go(seq, "exit");
      break;
    }
  }
  throw new Error(`stuck chips=${beforeC} bombs=${beforeB}`);
}
    ...
    `${JSON.stringify(
      {
        ...
        boldRouteHint:
          "Yellow → force E → bridges → red ice → green×2 → keys → bugs/left/sokoban → socket → block on brown → exit",
