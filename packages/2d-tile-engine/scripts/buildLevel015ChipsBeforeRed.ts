/**
 * From chips2+blue at (18,20): last chips nearby, then red, brown, exit.
 * Avoids south→north→south waste.
 */
...
function bfs(
  ...
  while (qi < q.length && n < maxNodes) {
    const f = q[qi++]!;
    n++;
    if (n % 100000 === 0) console.log(label, "…", n, "ms", Date.now() - t0);
    if (done(f.runner)) {
...
// Cut at chips2_blue: chips<=2, key_blue, at or after blue key pickup near (18,20)
...
) as { letters: string[] };
{
  ...
  if (cut < 0) {
    ...
    for (let i = 0; i < all.letters.length; i++) {
      ...
      if (
        ...
        rs2.playerState.tools.includes("ice_skates")
      ) {
        cut = i + 1;
        break;
      }
  ...
  if (cut < 0) throw new Error("chips2_blue cut missing");
  var letters = all.letters.slice(0, cut);
}
let r = applyLetters(letters);
save(letters, "chips2_blue", r);

// Both last chips, keep blue key
console.log("Searching chips0 keep blue");
{
  const seg = bfs(
    r,
    100,
...
console.log("Searching exit");
{
  ...
  if (!seg) {
    const seg2 = bfs(r, 50, 400_000, (x) => x.completed && !x.playerDied, "exit_any");
    if (!seg2) {
...
console.log("FINAL", {
  ...
  waits: finalLetters.filter((c) => c === "W").length,
  udlr: Object.fromEntries(
    ["U", "D", "L", "R"].map((c) => [c, finalLetters.filter((x) => x === c).length]),
  ),
});
    ...
    `${JSON.stringify(
      {
        ...
        boldRouteHint:
          "SW/NW boots; fire/ice; chips; blue; last chips; red; brown; exit; 89",
        moveVerified: true,
