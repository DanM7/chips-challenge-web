/**
 * ice→chips5→chips4→red@30,1→chips3→open red→c2b→chips→brown→exit≥89→pad
 */
...
const steps: { label: string; done: (x: Runner) => boolean }[] = [
  ...
  { label: "red", done: (x) => x.playerState.keys.includes("key_red") },
  {
    label: "chips3_keep_red",
    done: (x) =>
      x.playerState.chipsRemainingOnMap <= 3 && x.playerState.keys.includes("key_red"),
  },
    ...
    `${JSON.stringify(
      {
        ...
        boldRouteHint:
          "SW/NW boots; fire/ice; chips4; NE red; chips3; open exit; chips; brown; exit; 89",
        moveVerified: true,
