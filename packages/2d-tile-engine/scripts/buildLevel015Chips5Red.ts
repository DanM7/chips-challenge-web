/**
 * From ice: chips5+red near (18,4), open exit red, finish chips/brown/exit.
 */
...
) as { letters: string[] };
{
  ...
  if (cut < 0) throw new Error("ice missing");
...
const steps: { label: string; done: (x: Runner) => boolean }[] = [
  {
    label: "chips5_red",
      ...
      x.playerState.chipsRemainingOnMap <= 5 && x.playerState.keys.includes("key_red"),
  },
  {
    label: "open_red",
  ...
  {
    label: "c1327",
    done: (x) => cellTile(x.level, "upper", 13, 27) !== "chip",
  },
  {
    label: "c1619",
    done: (x) => cellTile(x.level, "upper", 16, 19) !== "chip",
  },
  {
    label: "brown",
...
for (const s of steps) {
  ...
  // Before brown, ensure blue door open / have blue
  if (s.label === "brown" && cellTile(r.level, "upper", 16, 11) === "door_blue") {
    if (!r.playerState.keys.includes("key_blue")) {
      console.log("Searching reblue");
      const seg = bfsPath(r, (x) => x.playerState.keys.includes("key_blue"), "reblue");
    ...
    `${JSON.stringify(
      {
        ...
        boldRouteHint:
          "SW/NW boots; fire/ice; red@chips5; open exit; chips; brown; exit; 89",
        moveVerified: true,
