/**
 * Cut at c2b (2 chips, blue, red door open): brown FIRST, then last chips, exit.
 */
...
};
{
  ...
  if (cut < 0) throw new Error("no c2b");
  var letters = all.letters.slice(0, cut);
}
let r = applyLetters(letters);
save(letters, "c2b", r);

console.log("Searching brown before last chips");
{
  const seg = bfsPath(
    r,
...
console.log("Searching last chips");
{
  const seg = bfsPath(
    r,
    ...
    `${JSON.stringify(
      {
        ...
        boldRouteHint:
          "SW/NW boots; fire/ice; NE red; open exit; brown; last chips; exit; 89",
        moveVerified: true,
