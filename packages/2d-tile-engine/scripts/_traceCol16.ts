...
const opener = expand("4R2L4D4U5RRUURRRDRUU");
for (const d of opener) stepMsCc1Simulation(r, d);

const seq = expand("4D2LURDR4U");
for (const d of seq) {
  const before = `${r.gx},${r.gy}`;
  const beforeTile = getCompositeTile(r.level, r.gx, r.gy);
  ...
  console.log(
    d[0]!.toUpperCase(),
    `${before}(${beforeTile})->${r.gx},${r.gy}(${getCompositeTile(r.level, r.gx, r.gy)})`,
    "blocks",
    b.join(" "),
    "16:",
    [20, 21, 22, 23, 24, 25, 26, 27, 28]
      .map((y) => `${y}=${getCompositeTile(r.level, 16, y).slice(0, 4)}`)
      .join(" "),
